...
for i=2:ny
    ...
    for i=1:ny-1
        for j=2:nx-1
            ...
            elseif phiCetild(i,j)>= 1/6 && phiCetild(i,j)<7/10
                phifetild(i,j) = 3*phiCetild(i,j)/4 +3/8;
            elseif  phiCetild(i,j)>= 0 && phiCetild(i,j)<= 5/6
                phifetild(i,j) = 3*phiCetild(i,j)/4 +3/8;
            elseif  phiCetild(i,j)>= 7/10 && phiCetild(i,j)<=1
                phifetild(i,j) = 1*phiCetild(i,j)/4 +2/3; 
            end
    ...
    for i=1
        for j=1:nx-1
            ...
            elseif phiCntild(i,j)>= 1/6 && phiCntild(i,j)<7/10
                phifntild(i,j) = 3*phiCntild(i,j)/4 +3/8;
            elseif  phiCntild(i,j)>= 0 && phiCntild(i,j)<= 5/6
                phifntild(i,j) = 3*phiCntild(i,j)/4 +3/8;
            elseif  phiCntild(i,j)>= 7/10 && phiCntild(i,j)<=1
                phifntild(i,j) = 1*phiCntild(i,j)/4 +2/3; 
            end
