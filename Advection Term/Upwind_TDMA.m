...
ny = 30; % Number of points along y
...
while error>tolerance && iter<max_iter
   ...
   for j=2
    ...
    for i=2:ny
        P(i,2) = -aE(i,2)/aC(i,2);
        Q(i,2) = (bC(i,2) - aN(i,2)*phi(i+1,2) - aS(i,2)*phi(i-1,2))/aC(i,2);


        for j=3:nx
            P(i,j) = -aE(i,j)/(aC(i,j) + aW(i,j)*P(i,j-1));
            Q(i,j) = (bC(i,j) - aS(i,j)*phi(i-1,j) - aW(i,j)*Q(i,j-1) - aN(i,j)*phi(i+1,j))/(aC(i,j) + aW(i,j)*P(i,j-1));
        end
