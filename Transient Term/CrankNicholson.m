...
%---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        for i=3:ny-1
            for j=3:nx-1
                ...
                aC(i,j) = -(aE(i,j) + aW(i,j) + aN(i,j) + aS(i,j)) + (ro * cp * A(i-1,j-1))/(2*dt);
                bC(i,j) = Sc(i,j) * A(i-1,j-1) + (ro * cp * A(i-1,j-1))*T(i,j,hour-1)/(2*dt);
        ...
        for j=2
            for i=3:ny-1
                ...
                aN(i,j) = - gamma_fn(i,j-1) * g_Diffn(i,j-1);
                aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
                aC(i,j) = -(aE(i,j) +  aN(i,j) + aS(i,j)) + (ro * cp * A(i-1,j-1))/(2*dt);
                bC(i,j) = Sc(i,j) * A(i-1,j-1) + (ro * cp * A(i-1,j-1))*T(i,j,hour-1)/(2*dt);
                aW(i,j) = 0;
            end
        end
        ...
        for i=2
            for j=3:nx-1
                ...
                aN(i,j) = - gamma_fn(i,j-1) * g_Diffn(i,j-1);
                aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
                aC(i,j) = -(aE(i,j) + aN(i,j) + aW(i,j) + aS(i,j)) + (ro * cp * A(i-1,j-1))/(2*dt);
                bC(i,j) = 0*Sc(i,j) * A(i-1,j-1) + (ro * cp * A(i-1,j-1))*T(i,j,hour-1)/(2*dt) - aS(i,j)*tiny;
                aS(i,j) = 0;
        ...
        for i=ny
            for j=3:nx-1
                ...
                aN(i,j) = - gamma_fn(i,j-1) * g_Diffn(i,j-1);
                aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
                aC(i,j) = -(aE(i,j) + aW(i,j) + aS(i,j) + aN(i,j)) + (ro * cp * A(i-1,j-1))/(2*dt);
                bC(i,j) = Sc(i,j) * A(i-1,j-1) + (ro * cp * A(i-1,j-1))*T(i,j,hour-1)/(2*dt) - aN(i,j)*tiny;
                aN(i,j) = 0;
        ...
        for j=nx
            for i=3:ny-1
                ...
                aN(i,j) = - gamma_fn(i,j-1) * g_Diffn(i,j-1);
                aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
                aC(i,j) = -(aW(i,j) + aN(i,j) + aS(i,j)) + (ro * cp * A(i-1,j-1))/(2*dt);
                bC(i,j) = Sc(i,j) * A(i-1,j-1) + (ro * cp * A(i-1,j-1))*T(i,j,hour-1)/(2*dt);
                aE(i,j) = 0;
...
%------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        for j=2
            for i=2
                ...
                aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
                aC(i,j) = -(aE(i,j)  + aN(i,j) + aS(i,j)) + (ro * cp * A(i-1,j-1))/(2*dt);
                bC(i,j) = Sc(i,j) * A(i-1,j-1) + (ro * cp * A(i-1,j-1))*T(i,j,hour-1)/(2*dt) - aS(i,j)*tiny;
                aW(i,j) = 0;
                aS(i,j) = 0;
...
%------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        for j=nx
            for i=ny
                ...
                aN(i,j) = - gamma_fn(i,j-1) * g_Diffn(i,j-1);
                aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
                aC(i,j) = -(aW(i,j) + aS(i,j) + aN(i,j)) + (ro * cp * A(i-1,j-1))/(2*dt);
                bC(i,j) = Sc(i,j) * A(i-1,j-1) + (ro * cp * A(i-1,j-1))*T(i,j,hour-1)/(2*dt) - aN(i,j)*tiny;
                aE(i,j) = 0;
                aN(i,j) = 0;
...
%------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        for j=2
            for i=ny
                ...
                aN(i,j) = - gamma_fn(i,j-1) * g_Diffn(i,j-1);
                aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
                aC(i,j) = -(aE(i,j)  + aS(i,j)+ aN(i,j)) + (ro * cp * A(i-1,j-1))/(2*dt);
                bC(i,j) = Sc(i,j) * A(i-1,j-1) + (ro * cp * A(i-1,j-1))*T(i,j,hour-1)/(2*dt) - aN(i,j)*tiny;
                aN(i,j) = 0;
                aW(i,j) = 0;
