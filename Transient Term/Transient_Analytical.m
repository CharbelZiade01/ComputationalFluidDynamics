...
%---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for i=1:ny-1
        ...
        for j=2:nx-1
            gamma_fe(i,j) = (1 - ge(i,j))*gamma_c(i+1,j) + ge(i,j)*gamma_c(i+1,j+1);
        end
    end
    ...
    for j=1:nx-1
        ...
        for i=2:ny-1
            gamma_fn(i,j) = (1 - gn(i,j))*gamma_c(i,j+1) + gn(i,j)*gamma_c(i+1,j+1);
        end
    end
...
%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 %---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    %
    %     Sfe(i,1) = Sc(i+1,1);
    %     Sfe(i,nx) = Sc(i+1,nx+1);
    %
    %     for j=2:nx-1
    %         Sfe(i,j) = (1 - ge(i,j))*Sc(i+1,j) + ge(i,j)*Sc(i+1,j+1);
    %     end
    % end
    %
    % % North
    ...
    %
    %     Sfn(1,j) = Sc(1,j+1);
    %     Sfn(ny,j) = Sc(ny+1,j+1);
    %
    %     for i=2:ny-1
    %         Sfn(i,j) = (1 - gn(i,j))*Sc(i,j+1) + gn(i,j)*Sc(i+1,j+1);
    %     end
    % end
...
%---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for i=3:ny-1
        for j=3:nx-1
            ...
            bC(i,j) = Sc(i,j) * A(i-1,j-1) + gamma_fe(i-1,j)*(grad_Txe(i-1,j)*Txe(i-1,j)+grad_Tye(i-1,j)*Tye(i-1,j)) + gamma_fe(i-1,j-1)*(grad_Txe(i-1,j-1)*Txe(i-1,j-1) + grad_Tye(i-1,j-1)*Tye(i-1,j-1)) + gamma_fn(i,j-1)*(grad_Txn(i,j-1)*Txn(i,j-1)+grad_Tyn(i,j-1)*Tyn(i,j-1)) + gamma_fn(i-1,j-1)*(grad_Txn(i-1,j-1)*Txn(i-1,j-1)+grad_Tyn(i-1,j-1)*Tyn(i-1,j-1));
    ...
    for j=2
        for i=3:ny-1
            ...
            bC(i,j) = Sc(i,j) * A(i-1,j-1) + gamma_fe(i-1,j)*(grad_Txe(i-1,j)*Txe(i-1,j)+grad_Tye(i-1,j)*Tye(i-1,j)) + gamma_fe(i-1,j-1)*(grad_Txe(i-1,j-1)*Txe(i-1,j-1) + grad_Tye(i-1,j-1)*Tye(i-1,j-1)) + gamma_fn(i,j-1)*(grad_Txn(i,j-1)*Txn(i,j-1)+grad_Tyn(i,j-1)*Tyn(i,j-1)) + gamma_fn(i-1,j-1)*(grad_Txn(i-1,j-1)*Txn(i-1,j-1)+grad_Tyn(i-1,j-1)*Tyn(i-1,j-1)) - 400*aW(i,j);
            aW(i,j) = 0;
        end
    end
    ...
    for i=2
        for j=3:nx-1
            ...
            aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
            aC(i,j) = -(aE(i,j) + aW(i,j) + aN(i,j) + aS(i,j));
            bC(i,j) = Sc(i,j) * A(i-1,j-1) + gamma_fe(i-1,j)*(grad_Txe(i-1,j)*Txe(i-1,j)+grad_Tye(i-1,j)*Tye(i-1,j)) + gamma_fe(i-1,j-1)*(grad_Txe(i-1,j-1)*Txe(i-1,j-1) + grad_Tye(i-1,j-1)*Tye(i-1,j-1)) + gamma_fn(i,j-1)*(grad_Txn(i,j-1)*Txn(i,j-1)+grad_Tyn(i,j-1)*Tyn(i,j-1)) + gamma_fn(i-1,j-1)*(grad_Txn(i-1,j-1)*Txn(i-1,j-1)+grad_Tyn(i-1,j-1)*Tyn(i-1,j-1)) - 320*aS(i,j);
            aS(i,j) = 0;
    ...
    for i=1:ny-1
        for j=1:nx
            fluxCb (i,j) = (h_inf * sqrt(Sxe(i,j)^2 + Sye(i,j)^2) + gamma_fe(i,j) * mod_Ee(i,j)/sqrt((Xfc(i+1,j+1)-Xfc(i+1,j))^2 + (Yfc(i+1,j+1)-Yfc(i+1,j))^2))/(h_inf * sqrt(Sxe(i,j)^2 + Sye(i,j)^2) * gamma_fe(i,j) * mod_Ee(i,j)/sqrt((Xfc(i+1,j+1)-Xfc(i+1,j))^2 + (Yfc(i+1,j+1)-Yfc(i+1,j))^2));
            fluxVb (i,j) = - fluxCb(i,j) * T_inf - (h_inf * sqrt(Sxe(i,j)^2 + Sye(i,j)^2) * gamma_fe(i,j) * (grad_Txe(i,j)*Txe(i,j) + grad_Tye(i,j)*Tye(i,j)))/(h_inf * sqrt(Sxe(i,j)^2 + Sye(i,j)^2) * gamma_fe(i,j) * mod_Ee(i,j)/sqrt((Xfc(i+1,j+1)-Xfc(i+1,j))^2 + (Yfc(i+1,j+1)-Yfc(i+1,j))^2));
        end
    end
    ...
    for j=nx
        for i=3:ny-1
            ...
            aN(i,j) = - gamma_fn(i,j-1) * g_Diffn(i,j-1);
            aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
            aC(i,j) = -(aW(i,j) + aN(i,j) + aS(i,j)) + fluxCb(i-1,j);
            bC(i,j) = Sc(i,j) * A(i-1,j-1) + gamma_fe(i-1,j)*(grad_Txe(i-1,j)*Txe(i-1,j)+grad_Tye(i-1,j)*Tye(i-1,j)) + gamma_fe(i-1,j-1)*(grad_Txe(i-1,j-1)*Txe(i-1,j-1) + grad_Tye(i-1,j-1)*Tye(i-1,j-1)) + gamma_fn(i,j-1)*(grad_Txn(i,j-1)*Txn(i,j-1)+grad_Tyn(i,j-1)*Tyn(i,j-1)) + gamma_fn(i-1,j-1)*(grad_Txn(i-1,j-1)*Txn(i-1,j-1)+grad_Tyn(i-1,j-1)*Tyn(i-1,j-1)) - fluxCb(i-1,j)*T_inf ;
            aE(i,j) = 0;
...
%------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for j=2
        for i=2
            ...
            aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
            aC(i,j) = -(aE(i,j) + aW(i,j) + aN(i,j) + aS(i,j));
            bC(i,j) = Sc(i,j) * A(i-1,j-1) + gamma_fe(i-1,j)*(grad_Txe(i-1,j)*Txe(i-1,j)+grad_Tye(i-1,j)*Tye(i-1,j)) + gamma_fe(i-1,j-1)*(grad_Txe(i-1,j-1)*Txe(i-1,j-1) + grad_Tye(i-1,j-1)*Tye(i-1,j-1)) + gamma_fn(i,j-1)*(grad_Txn(i,j-1)*Txn(i,j-1)+grad_Tyn(i,j-1)*Tyn(i,j-1)) + gamma_fn(i-1,j-1)*(grad_Txn(i-1,j-1)*Txn(i-1,j-1)+grad_Tyn(i-1,j-1)*Tyn(i-1,j-1)) - 400*aW(i,j) - 320*aS(i,j);
            aW(i,j) = 0;
            aS(i,j) = 0;
...
%------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for j=nx
        for i=ny
            ...
            aN(i,j) = - gamma_fn(i,j-1) * g_Diffn(i,j-1);
            aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
            aC(i,j) = -(aW(i,j) + aS(i,j))  + fluxCb(i-1,j);
            bC(i,j) = Sc(i,j) * A(i-1,j-1) + gamma_fe(i-1,j)*(grad_Txe(i-1,j)*Txe(i-1,j)+grad_Tye(i-1,j)*Tye(i-1,j)) + gamma_fe(i-1,j-1)*(grad_Txe(i-1,j-1)*Txe(i-1,j-1) + grad_Tye(i-1,j-1)*Tye(i-1,j-1)) + gamma_fn(i,j-1)*(grad_Txn(i,j-1)*Txn(i,j-1)+grad_Tyn(i,j-1)*Tyn(i,j-1)) + gamma_fn(i-1,j-1)*(grad_Txn(i-1,j-1)*Txn(i-1,j-1)+grad_Tyn(i-1,j-1)*Tyn(i-1,j-1)) - fluxCb(i-1,j)*T_inf ;
            aE(i,j) = 0;
            aN(i,j) = 0;
        end
...
%------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for j=nx
        for i=2
            ...
            aS(i,j) = - gamma_fn(i-1,j-1) * g_Diffn(i-1,j-1);
            aC(i,j) = -(aW(i,j) + aN(i,j)) + aS(i,j) + fluxCb(i-1,j);
            bC(i,j) = Sc(i,j) * A(i-1,j-1) + gamma_fe(i-1,j)*(grad_Txe(i-1,j)*Txe(i-1,j)+grad_Tye(i-1,j)*Tye(i-1,j)) + gamma_fe(i-1,j-1)*(grad_Txe(i-1,j-1)*Txe(i-1,j-1) + grad_Tye(i-1,j-1)*Tye(i-1,j-1)) + gamma_fn(i,j-1)*(grad_Txn(i,j-1)*Txn(i,j-1)+grad_Tyn(i,j-1)*Tyn(i,j-1)) + gamma_fn(i-1,j-1)*(grad_Txn(i-1,j-1)*Txn(i-1,j-1)+grad_Tyn(i-1,j-1)*Tyn(i-1,j-1)) -320*aS(i,j) - fluxCb(i-1,j)*T_inf ;
            aE(i,j) = 0;
            aS(i,j) = 0;
